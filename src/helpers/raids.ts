...
import { getAllOfficersMembers, getAllRaidersMembers, getDiscordChannel, getDiscordRoleByName } from "./discord";
import { createRaidHelperEvent, deleteRaidHelperEvent, fetchRaidHelperEventSignUps, fetchRaidHelperPostedEvents, type PostedRaidHelperEvent } from "./raid-helper";
...
export async function recreateNextWeekRaidHelper(channelIdToRecreateRh: string, removedEventStartTime: number) {
  try {
    ...
    nextRaidDate.setHours(Number(process.env.RAID_HOUR_START as string), Number(process.env.RAID_MINUTE_START as string), 0, 0); // Set hours and minutes
    ...
    console.log("Next raid created on:", nextRaidDate.toLocaleString('fr-FR', {
      weekday: 'long',
      year: 'numeric',
      month: 'long',
      day: 'numeric',
...
export async function recreateRaidHelperChannelThread(client: Client, channelIdToRecreateRh: string, raidDate: Date) {
  try {
    ...
    const raidHelperMessage = channelMessages.find(async message => message.author.id === process.env.RAID_HELPER_ID);
    ...
    const thread = await raidHelperMessage.startThread({
      name: raidDate.toLocaleString('fr-FR', { day: 'numeric', year: 'numeric', month: 'numeric' }),
    });
